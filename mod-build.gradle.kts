...
// Access widener
//
// Biome.climateSettings is private and Biome$ClimateSettings is package-private, so the
// public downfall() accessor on that record cannot be reached from a mod. Vanilla offers
// getBaseTemperature() and hasPrecipitation() but nothing at all for downfall, which is why
// the biome tools used to report a hardcoded 0.0f.
//
// The widener is GENERATED per version rather than checked in, because the two Loom plugins
// disagree about the namespace header even though the names themselves are identical (every
// target resolves to Mojang official names). Legacy Loom on 1.21.x demands `named` and
// rejects `official` with "Namespace mismatch, expected named got official"; LoomNoRemap on
// 26.1+ demands `official` and rejects `named` with "Expected official namespace for access
// widener entry". Only the header token differs, so we emit the right one for each build.
//
// This runs at configuration time because Loom reads accessWidenerPath while configuring.
run {
    val awNamespace = if (mappingsType == "mojmap_native") "official" else "named"
    val awFile = layout.buildDirectory
        .file("generated/accesswidener/minecraft_fabric_mcp.accesswidener").get().asFile
    awFile.parentFile.mkdirs()
    val tab = "\t"
    val climate = "net/minecraft/world/level/biome/Biome\u0024ClimateSettings"
    awFile.printWriter().use { w ->
        w.println("accessWidener" + tab + "v2" + tab + awNamespace)
        w.println()
        w.println("# Generated by mod-build.gradle.kts -- do not edit by hand.")
        w.println("# Grants read access to Biome.climateSettings so downfall() is reachable;")
        w.println("# vanilla exposes no accessor for it. Namespace for this target: $awNamespace")
        w.println("accessible" + tab + "class" + tab + climate)
        w.println(
            "accessible" + tab + "field" + tab +
                "net/minecraft/world/level/biome/Biome" + tab + "climateSettings" + tab +
                "L" + climate + ";"
        )
    }
    // The generated widener must also be INSIDE the jar: fabric.mod.json names it, and the
    // loader resolves it from the mod jar at runtime. Leaving it out is not a build error on
    // LoomNoRemap -- 26.x jars built clean with no widener entry at all and would only have
    // failed when the loader tried to read it. Legacy Loom catches it at remapJar. Copy it in
    // explicitly so every target agrees.
    tasks.named("processResources", Copy::class.java) { from(awFile) }

    val loomExt = project.extensions.findByName("loom")
    if (loomExt != null) {
        val getter = loomExt.javaClass.methods.firstOrNull { it.name == "getAccessWidenerPath" }
        if (getter != null) {
            val fileProperty = getter.invoke(loomExt)
            fileProperty.javaClass.methods
                .firstOrNull { it.name == "set" && it.parameterTypes.firstOrNull() == java.io.File::class.java }
                ?.invoke(fileProperty, awFile)
            logger.lifecycle("Access widener ($awNamespace) wired for $mcVersion")
        } else {
            logger.warn("Loom on $mcVersion exposes no accessWidenerPath; downfall will not widen")
        }
    }
}
